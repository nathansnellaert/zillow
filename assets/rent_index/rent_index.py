"""Fetch and process Zillow Rent Index data"""
import pyarrow as pa
from utils.http_client import get
from utils.io import load_state, save_state
from datetime import datetime, timezone

def fetch_rent_index():
    """Fetch Zillow Rent Index data"""
    url = "https://files.zillowstatic.com/research/public_csvs/zori/Metro_zori_uc_sfrcondomfr_sm_month.csv"
    response = get(url)
    response.raise_for_status()
    return response.text

def process_rent_index():
    """Process Zillow Rent Index data"""
    import pandas as pd
    from io import StringIO
    
    # Load state
    state = load_state("rent_index")
    
    # Fetch data
    csv_data = fetch_rent_index()
